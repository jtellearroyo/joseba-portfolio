...
function ControlDiarioView() {
  ...
  return (
    <div>
      <div className="rounded-lg border border-sky-100 bg-sky-50 px-3 py-2.5 text-[12.5px] text-sky-800">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2.5 sm:grid-cols-3">
        <Stat label="Tareas pendientes" value={TASKS.length} sub="Últimos 30 días" />
        <Stat label="Asistencia" value="2" sub="Por resolver" />
        <Stat label="Sesiones" value="0" sub="Sin anotar" />
      </div>
        ...
        <table className="w-full min-w-[520px] text-left text-[13px]">
          <thead>
            <tr className="border-b border-slate-200 text-[10.5px] uppercase tracking-wide text-slate-400">
              ...
              <th className="px-3 py-2.5 font-medium">Terapeuta</th>
            ...
            {rows.map((t) => (
              <tr key={t.id} className="border-b border-slate-100 last:border-0">
                <td className="px-3 py-2.5">
                  ...
                  <p className="text-[11.5px] text-slate-400">{t.tag}</p>
                </td>
                ...
                <td className="px-3 py-2.5 text-slate-600">{t.therapist}</td>
                <td className="px-3 py-2.5">
                  <span className="task-badge">Revisar asistencia</span>
                  <p className="mt-1 max-w-[28ch] text-[11.5px] text-slate-500">
                    Si estuvo, cambia a Atendido. Si no vino, márcalo como No vino.
                  </p>
                </td>
              </tr>
            ))}
...
function KpisView() {
  ...
  return (
    <div>
      ...
      <div className="mt-4 rounded-lg border border-indigo-100 bg-indigo-50 p-3.5">
        <p className="text-[11.5px] font-semibold uppercase tracking-wide text-indigo-600">Resumen del periodo</p>
        <ul className="mt-2 space-y-1.5 text-[12.5px] text-indigo-900">
          {KPI_SUMMARY.map((s) => (
      ...
      <p className="mt-5 font-mono text-[10.5px] uppercase tracking-wide text-slate-400">Actividad clínica</p>
      ...
      <p className="mt-5 font-mono text-[10.5px] uppercase tracking-wide text-slate-400">Centro y finanzas</p>
      <div className="mt-2.5 grid grid-cols-2 gap-2.5 sm:grid-cols-3">
        {KPI_FINANCE.map((k) => (
          <Stat key={k.label} {...k} />
        ))}
  ...
  return (
    <div>
      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
        ...
        <Stat label="7 días" value="83" />
        <Stat label="Pendientes" value="0" />
        <Stat label="Este mes" value="336" />
      </div>

      <div className="mt-4 flex flex-wrap items-center gap-2">
        <input type="text" placeholder="Buscar paciente, teléfono..." className="demo-input min-w-[180px] flex-1" />
        <span className="filter-pill filter-pill-active">Hoy</span>
        <span className="filter-pill">Estados</span>
      </div>
          ...
          <tbody>
            {BOOKINGS.map((b, i) => (
              <tr key={i} className="border-b border-slate-100 last:border-0">
                <td className="px-3 py-2.5 text-slate-600">{b.time}</td>
                <td className="px-3 py-2.5 font-medium text-slate-900">{b.patient}</td>
                <td className="px-3 py-2.5 text-slate-600">{b.treatment}</td>
                <td className="px-3 py-2.5 text-slate-600">{b.therapist}</td>
                <td className="px-3 py-2.5">
                  <span className={`rounded-full px-2 py-1 text-[11.5px] font-medium ${STATUS_STYLE[b.status]}`}>{b.status}</span>
                </td>
              </tr>
            ))}
          </tbody>
...
function PacientesView({ onSelect }) {
  return (
    <div>
      <input type="text" placeholder="Buscar paciente..." className="demo-input w-full max-w-xs" />
      <div className="mt-4 overflow-x-auto rounded-lg border border-slate-200 bg-white">
        <table className="w-full min-w-[460px] text-left text-[13px]">
          <thead>
            <tr className="border-b border-slate-200 text-[10.5px] uppercase tracking-wide text-slate-400">
              <th className="px-3 py-2.5 font-medium">Paciente</th>
              <th className="px-3 py-2.5 font-medium">Última visita</th>
              <th className="px-3 py-2.5 font-medium">Estado</th>
          ...
          <tbody>
            {PATIENTS.map((p) => (
              <tr key={p.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                ...
                <td className="px-3 py-2.5 text-slate-600">{p.lastVisit}</td>
                <td className="px-3 py-2.5">
                  <span className="rounded-full bg-slate-100 px-2 py-1 text-[11.5px] font-medium text-slate-600">{p.status}</span>
                </td>
                <td className="px-3 py-2.5 text-right">
                  <button onClick={() => onSelect(p)} className="text-[12px] font-medium text-[var(--signal)] hover:underline">
                    Ver ficha →
                  </button>
                </td>
              </tr>
...
function PatientDetail({ patient, onBack }) {
  ...
  return (
    <div>
      <button onClick={onBack} className="text-[12.5px] font-medium text-slate-500 hover:text-slate-900">
        ← Volver al listado
      </button>

      <div className="mt-3 rounded-lg border border-slate-200 bg-white p-4">
        <div className="flex items-center gap-3">
          <span className="flex h-10 w-10 items-center justify-center rounded-full bg-amber-50 text-[14px] font-semibold text-amber-700">
            {initials}
          </span>
          <div>
            <p className="text-[15px] font-semibold text-slate-900">{patient.name}</p>
        ...
        <p className="mt-5 text-[11px] font-semibold uppercase tracking-wide text-slate-400">Datos personales</p>
        ...
        <label className="demo-field mt-3.5">
          <span>Observaciones</span>
          <textarea rows={2} defaultValue={patient.notes} />
        </label>
      </div>
    </div>
  );
...
function PlaceholderView({ label }) {
  return (
    <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-slate-300 bg-white py-14 text-center">
      <p className="text-[14px] font-medium text-slate-500">{label} no está incluido en esta demo</p>
      <p className="mt-2 max-w-xs text-[12.5px] text-slate-400">
        En la versión real funciona con el mismo patrón que las demás pestañas.
      </p>
    </div>
  );
}
...
export default function CrmDemo({ variant = 'embed' }) {
  ...
  return (
    <div className={`crm-demo bg-[#f4f5f7] text-slate-900 ${full ? '' : 'overflow-hidden border border-[var(--line)]'}`}>
      <div className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-2.5">
        <div className="flex items-center gap-2">
          <DemoLogo />
          <div>
            <p className="text-[12px] font-semibold leading-tight text-slate-900">CRM DEMO</p>
            <p className="text-[10px] leading-tight text-slate-400">Clínica Ejemplo</p>
          </div>
        </div>
        <span className="rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 font-mono text-[9.5px] text-amber-700">
          Datos ficticios
        </span>
      </div>

      <div className={`flex flex-col ${full ? 'lg:flex-row' : 'sm:flex-row'}`}>
        <nav
          className={`flex gap-1 overflow-x-auto border-b border-slate-200 bg-white px-3 py-2 ${
            full
              ? 'lg:w-[220px] lg:shrink-0 lg:flex-col lg:gap-0.5 lg:overflow-visible lg:border-b-0 lg:border-r lg:px-3 lg:py-5'
              : 'sm:w-[168px] sm:shrink-0 sm:flex-col sm:gap-0.5 sm:overflow-visible sm:border-b-0 sm:border-r sm:px-3 sm:py-4'
          }`}
        >
          {NAV.map((n) => (
            <button
              ...
              className={`nav-btn text-[12.5px] ${active === n.id ? 'nav-btn-active' : ''}`}
            >
              <NavIcon name={n.icon} />
              {n.label}
            </button>
        ...
        <div className={full ? 'flex-1 p-5 sm:p-7' : 'max-h-[480px] flex-1 overflow-y-auto p-4 sm:p-5'}>
          <h3 className={full ? 'text-[22px] font-semibold tracking-tight text-slate-900' : 'text-[16px] font-semibold tracking-tight text-slate-900'}>
            {activeNav.label}
          </h3>
          <p className="mt-0.5 text-[12.5px] text-slate-500">{TAB_SUBTITLES[active]}</p>
